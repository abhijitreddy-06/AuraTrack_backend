...
export const runHabitReminderJob = async () => {
  ...
  try {
    const habits = await Habit.findAll({
      attributes: ["user_id", "last_completed_date"],
    });
    const userIds = [
      ...new Set(
        habits
          .filter((habit) => habit.last_completed_date !== today)
          .map((habit) => habit.user_id),
      ),
    ];
    ...
    console.error("Habit reminder job failed:", error);
  }
};
...
export const runBirthdayReminderJob = async () => {
  ...
  try {
    const birthdays = await Birthday.findAll({
      attributes: ["user_id", "name", "date"],
    });
    const namesByUser = new Map();
    birthdays
      .filter((birthday) => birthday.date.slice(5) === tomorrow.slice(5))
      .forEach((birthday) => {
        namesByUser.set(birthday.user_id, [
          ...(namesByUser.get(birthday.user_id) || []),
          birthday.name,
        ]);
      });
    await Promise.allSettled(
      [...namesByUser.entries()].map(([userId, names]) => {
        const people =
          names.length === 1
            ? names[0]
...
export const runTodoCleanupJob = async () => {
  try {
    ...
    console.error("IST todo cleanup job failed:", error);
  }
};
...
export const startNotificationScheduler = () => {
  ...
  cron.schedule("50 14 * * *", () => void runHabitReminderJob(), {
